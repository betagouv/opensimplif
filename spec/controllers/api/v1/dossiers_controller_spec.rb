  ...
  describe 'GET show' do
    ...
    context 'when procedure is found and belongs to current admin' do
      ...
      context 'when dossier exists and belongs to procedure' do
        ...
        let(:dossier_id) { dossier.id }
        let(:body) { JSON.parse(retour.body, symbolize_names: true) }
        let(:field_list) { %i[id created_at updated_at archived mandataire_social entreprise etablissement cerfa types_de_piece_justificative pieces_justificatives champs commentaires state] }
